...
function setCustomersOfCarparks($worldid,$city) {
	...
	if($r_carparks = $db->query("SELECT * FROM grounds WHERE state=3 AND city='".$city["hex"]."' AND worldid=".$worldid)) {
		while($gnd = $r_carparks->fetch_array(MYSQLI_ASSOC)) {
			...
			$cust = 10*$city["strdichte"]*5*$city["population"]*$restaurants*$strassen/pow(count($nbh),2);
	...
	if($r_rests = $db->query("SELECT * FROM grounds WHERE state=2 AND city='".$city["hex"]."' AND worldid=".$worldid)) {
		while($gnd = $r_rests->fetch_array(MYSQLI_ASSOC)) {
			...
			/* 	Der Gegner mit voller Kundenanzahl und 
				höchster Attraktivität wird in der Betrachtung ausgelassen.*/
			...
			//$plr = getPlayer($gnd["ownerid"]); alert($plr["name"]." in ".$city["name"]."=".$customers."/".$custOfCity);
...
function setAllKontos($worldid) {
	...
	// outgo mit others tauschen, outgo ist am Ende gleich 0
...
function setCustomersOfHotels($worldid,$city) {
	...
	$cpop = countPatternsInMap($cmap,"HG","HH","HI")*$city["popdichte"];
	...
	$query = "UPDATE cities SET population=".($cpop+$allemieter)." WHERE hex='".$city["hex"]."'";
	$db->query($query) or die("Error during setCustomersOfHotels: ".$query);
	$db->Close();
}
...
function makeTick($worldid, $hours) {
	...
	while(time()>getNextTick($worldid)) {
		...
		$db->query("UPDATE world SET log=\"$log\" WHERE id=".$worldid)	
			or die("error logging");
	}
	...
	$plr = $r_plr->fetch_array(MYSQLI_ASSOC); // stärksten Spieler auslassen
	while($plr = $r_plr->fetch_array(MYSQLI_ASSOC)) {
		if($plr["RVvote"])
	...
	if($votes4restart/$votes4NOrestart>0.5)
		restartGame($worldid);
...
function restartGame($worldid) {
	...
	if(!table_exists("highscore")) {
		...
		$db->query("INSERT INTO highscore (round) VALUES ((SELECT round FROM highscore WHERE 1 ORDER BY round DESC LIMIT 1)+1)");
		$db->query("UPDATE world SET round=(SELECT round FROM highscore WHERE 1 ORDER BY round DESC LIMIT 1) WHERE 1");
	}
	...
	$db->query("DELETE messages WHERE 1");
